...
For simplicity, we refer to all of these events involving protein-protein interactions as CCC.

Cells are not independent entities in biology, they react to stimuli coming
from their environment. Cells can react to chemical cues, including such from 
other cells, that we will refer to as cell-cell communication events (CCC). 
These cell-cell signaling events can trigger crucial biological processes
like apoptosis or cell migration, making the study of them relevant to better
understand diseases. 

CCC events are essential for homeostasis, development, and disease,
and their estimation is becoming a routine approach in scRNA-seq data analysis.

CCC commonly refers to interactions between secreted ligands and plasma membrane receptors.
However, picture can be broadened to include secreted enzymes, extracellular matrix proteins,
transporters, and interactions that require the physical contact between cells, 
such as cell-cell adhesion proteins and gap junctions.

Furthermore, CCC communication events ellicit a downstream response, such as the
induction of known pathways or transcription factors.
![](src/ccc_overview.png){width=60%}

Although different types of tools to infer CCC events from single-cell data exist,
they often focus either on ligand-receptor predictions alone, or such which 
also connect to downstream signaling. Here, we will showcase both.

## Set-up Env

Namely, install and the two CCC packages that we will use: LIANA and NicheNet.
...
such which aim to prioritize ligand-receptor interactions alone.

There is an ever-growing  number of methods aimed at this particular inference task.
We recently have harmonized seven of them, along with 16 ligand-receptor prior knowledge resources
into the ligand-receptor framework - [LIANA](https://saezlab.github.io/liana/articles/liana_tutorial.html).
...
[NATMI](https://www.nature.com/articles/s41467-020-18873-z), each with it's matching resource.

As is generally the case, we will apply those methods to predict interactions from
...
As such, we will create an object with COVID-19 infected blood cells alone.
```{r}
...
a score which also aims at inferring interaction specificity.
...
Strangely, see very little agreement between the tools, even though despite
their different algorithms and resources,
both aim at predicting the most relevant interactions.
...
![](src/ccc_limitations.png){width=90%}
