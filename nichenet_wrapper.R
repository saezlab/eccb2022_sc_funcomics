nichenet_covid19_wrapper = function(receiver, DE_table, sobj,  liana_res, ligand_target_matrix){
  ...
  potential_ligands <- liana_res %>% filter(target == receiver) %>% pull(ligand.complex) %>% unique()  %>% intersect(colnames(ligand_target_matrix)) # only keep common ligands
  
  geneset_oi <- DE_table %>% filter(contrast == receiver)  %>% filter(pval < 0.05 & abs(logFC) > 0.50) %>% pull(name) %>% unique()
  ...
  liana_nichenet <- liana_nichenet %>% left_join(DE_table %>% rename(source = contrast, ligand.complex = name, logFC_ligand = logFC, pval_ligand = pval)) %>% left_join(DE_table %>% rename(target = contrast, receptor.complex = name, logFC_receptor = logFC, pval_receptor = pval))
  
  return(list(liana_nichenet = liana_nichenet, p_ligand_expression = p_ligand_expression, p_ligand_target_network = p_ligand_target_network, p_liana_nichenet = p_liana_nichenet, p_nichenet_liana = p_nichenet_liana))
